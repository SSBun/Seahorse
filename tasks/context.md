...
- GitHub 最新 Release 为 `v1.13.0`；其 Apple Development 签名 DMG 与 SHA256 已公开，Sparkle appcast 已发布 build `13`，DMG SHA256 为 `066ea1d4b749882b575dac5759e5a849e7e9c5ee4f9a297c6bc06b234fbd7570`。
...
- 根目录 `@ssbun/seahorse` npm wrapper 本地 manifest 为 `1.13.0`，只发布 `install.js` 与 README，并按自身版本从 GitHub Release 下载 `Seahorse-<version>.dmg`；npm registry 的 `latest` 为 `1.13.0`，分发包只支持 Apple Silicon macOS。

...
## 2026-07-28 — 先区分持续运行退化与单次交互开销

Trigger:
- 用户描述功能在应用启动时很快、运行一段时间后逐渐变慢。

Rule:
- 必须比较新启动进程与长时间运行进程的同一路径，并检查随时间累积的状态、任务、订阅、缓存和资源。
- 不得仅凭当前热路径的静态开销，把问题改写成每次交互都存在的性能问题。

Check:
- 修复前记录至少一个能随运行时间或操作次数变化的指标，并证明它与退化同步。
- 修复后验证该指标不再无界增长，且原始操作的延迟保持稳定。

- 优化 SwiftUI 列表滚动图片时，必须区分“已显示的图片”和“尚未完成的加载”：滚动中保留已显示内容，只暂停或取消 pending 与新加载，不能用全局开关把所有图片 View 一起替换成占位图。

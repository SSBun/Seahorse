...
- [ ] T4：`main` 与 `v1.13.0` 推送到 `origin`，公开 GitHub Release 提供可下载且摘要一致的 DMG 与 SHA256。
- [ ] T5：公开 Sparkle appcast 提供 `1.13.0 (13)`，下载 URL、长度和 EdDSA 签名与 GitHub 最终 DMG 一致。
- [ ] T6：`@ssbun/seahorse@1.13.0` 发布到 npm，`latest` 与公开 tarball 验证通过。
- [ ] T7：发布记录完整，远端分支/tag/Release/npm 状态一致，工作区只保留已明确说明的生成物或无关状态。
